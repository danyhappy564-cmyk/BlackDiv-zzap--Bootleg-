...
- 2026-09-24 22:12 (KST) — **"도망가서 엎드린 봇이 총 맞아도 반응 없던 버그"의
  근본 원인을 찾아서 고침.** SAIN은 적을 "눈으로 보거나 소리로 들었을 때"만 그
  적의 위치를 기억하고, **몸에 직접 총을 맞았을 때는 쏜 적을 기억하는 처리가 아예
  빠져 있었음**(SAIN 코드에 "맞아서 알게 된 위치"라는 항목이 만들어만 져 있고 한
  번도 안 쓰임). 그래서 안 보이는 곳에서 쏘는 적한테 맞으면 SAIN 전투 AI가 꺼진
  채로 있었음. 이제 블디 봇이 맞는 순간 쏜 적의 위치를 SAIN에 알려줘서 SAIN이
  바로 전투 모드로 돌아와 엄폐·반격까지 함. (09-18부터 넣었던 "쏜 쪽으로 몸만
  돌리기"는 이 수정이 안 먹히는 예외용 보조 장치로 남겨둠.) 순찰 안전망이
  실제로 작동했는지도 로그로 확인할 수 있게 함.
- 2026-09-24 21:55 (KST) — 릴리즈용 압축파일(Release 빌드 시 자동 생성되는
  zip)에 블디 dll만 들어가야 하는데, 빌드할 때 참고용으로만 쓰는 게임 본체
  파일·SAIN·BepInEx·Unity 파일 80여 개가 전부 같이 들어가던 문제 고침. 그대로
  풀었으면 플러그인 폴더에 SAIN이 하나 더 생기는 등 게임이 깨질 수 있었음.
  게임 폴더에 직접 복사되는 dll(`BepInEx\plugins\BlackDiv\BlackDiv.dll`)은
  원래부터 정상이었음.
...
| `BDHitRevealsShooter` | 블디 봇이 보지도 듣지도 못한 적한테 총을 맞으면 SAIN이 그 적을 계속 "모르는 적"으로 둬서 반격을 안 하던 문제의 근본 수정 — 맞는 순간 쏜 적을 SAIN에 알려줘서 SAIN 전투 AI가 복귀 (아래 26/09/24 항목) |
| `BDUnderFireSteeringFallback` | 위 수정으로도 SAIN이 복귀 못 하는 예외 상황용 보조 장치 — 최소한 쏜 쪽으로 몸은 돌림 (26/09/18, 09/24 v3) |
